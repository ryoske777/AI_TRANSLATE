...
- **통합 시트** *(기본 켜짐)* — 모든 파일을 한 시트에 몰아서 출력

### 통합 시트
기본으로 켜져 있습니다. `통합` 시트가 맨 앞에 추가됩니다. (파일별 개별 시트도 그대로 유지 / 끄려면 체크 해제)

- type 열 개수와 언어 목록을 **전체 공통으로 통일**하므로 `ko-KR` 은 시트 전체에서 항상 같은 열.
  전체 범위 필터·정렬이 가능합니다. 대신 그 파일에 없는 언어는 빈 칸으로 남습니다.
- 각 파일이 시작되는 행에 **파일명이 적힌 머리글 행**이 들어갑니다. 빈 행은 넣지 않아
  데이터가 끊기지 않습니다.
- '각각 저장' 을 누르면 `통합.xlsx` 도 함께 받아집니다.
