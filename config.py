...
#   결과열/특이사항열 머리글 자동 기입 — 데이터 시작 행 바로 위(보통 1행)에
#   'pt-BR', 'pt-BR 특이사항' 처럼 적어, 어느 열이 무슨 언어인지 보이게 한다.
#   사용자가 직접 적어 둔 머리글은 덮지 않는다.
WRITE_HEADER = True
...
#   표기가 용어집과 '완전히 동일'한 자리에는 보호 등급과 무관하게 «T:» 마커를
#   씌운다. 셀 전체가 용어이거나, 문장 속에서 앞뒤가 단어 경계인 채 표기가
#   같은 경우(조사만 붙은 형태 포함)가 대상이다. 끄면 예전처럼 HARD 등급만
#   마커가 붙어, 용어집에 있는 SOFT/HINT 단어가 마커 없이 직역돼 나간다.
GLOSSARY_MARK_EXACT_ALWAYS = True
