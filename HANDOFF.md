...
### 결정사항 (사용자 확인)
- **대상 등급은 HARD 뿐** (`glossary.MARK_LEVELS_DEFAULT`). SOFT 는 '권장(굴절 허용)'
  등급이라 마커로 굳히면 성·수·격이 막혀 문장이 깨진다. 설정으로 바꿀 수 있다.
...
| `GLOSSARY_MARK_LEVELS` | `["HARD"]` | 어느 보호 등급까지 씌울지 |
| `GLOSSARY_MARK_WRITE_BACK` | True | 씌운 마커를 입력 열에도 기입할지 |

설정창(⚙) 용어집 항목에 체크박스 2개로 노출. settings.json 에 저장된다.
